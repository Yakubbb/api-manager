...
const SystemPromptContent: React.FC<SystemPromptContentProps> = ({
    contents,
    isEditable,
    handleInputChange,
    updateStatus
}) => {
    const placeholderText = `Введите текст системного запроса здесь...`;
    const displayContent = typeof contents === 'string' ? contents : JSON.stringify(contents, null, 2);

    return (
        <div className="flex flex-col h-full">
            <h2 className="text-xl font-semibold mb-3 border-b pb-2">Системный запрос</h2>
            {isEditable ? (
                <textarea
                    name="contents"
