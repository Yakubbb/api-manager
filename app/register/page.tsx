"use client"
import { createUser } from "@/server-side/database-handler"; // Предполагается, что это ваша серверная функция
import { useActionState } from "react";
import { MdError, MdCheckCircle } from "react-icons/md";
import Link from 'next/link';
import { FaRegUser, FaLock } from "react-icons/fa"; // Иконки для логина и пароля
import { GiSandsOfTime } from "react-icons/gi"; // Иконка для состояния загрузки
        ...
        <div className="flex flex-col gap-8">
          ...
          {state?.type !== 'ok' && (
            <div className="mt-4 text-center text-sm text-gray-600">
              Уже есть аккаунт?
              <Link href="/" className="ml-1 font-semibold text-[#7242f5] hover:text-[#5a2ed1] transition-colors duration-200">
                Войти
              </Link>
            </div>
          )}
